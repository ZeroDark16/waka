...
function createUpdateTableModal(tableId = null) {
    ...
    const body = ''+
        '<table>'+
            ...
            '</tr>'+
        '</table>'+
        '<span class="modal-message">Seleccione la tabla y su nuevo estado"</span>';

    const modal = createModalFromBody(body, "Modificar mesa");
...
addButton.onclick = function() {
    ...
    request.onload = function () {
        if(this.status === 200) {
            ...
            createModalMessage("Error Code: "+this.status, "Transaccion fallida", "An error has occurred during your proccess.", "error");
        }
    }
    request.onerror = function () {
        createModalMessage("Error Code: "+this.status, "Un error ha ocurrido", "An error has occurred during your proccess.", "error");
    }
...
function addTable(table) {
    ...
    request.onload = function () {
        if(this.status === 200) {
            createModalMessage("Agregar tabla", "Tabla agregada", JSON.parse(this.responseText)["response"], "success");
            refreshTables();
        }
        else {
            const cause = JSON.parse(this.responseText);
            createModalMessage("Error", "Un error ha ocurrido", cause["response"], "error")
        }
    }
